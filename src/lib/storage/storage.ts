import { localStorageAPI } from './local';
import type { ModelItem, Conversation, ImageHistoryItem, AppStorage } from '@/types';
...
export function getStorage(): AppStorage {
  ...
  try {
    const data = localStorageAPI.get<string>(STORAGE_KEY);
    if (data) {
...
export function saveStorage(storage: AppStorage): void {
  localStorageAPI.set(STORAGE_KEY, storage);
}
...
export function groupConversationsByDate(conversations: Conversation[]): { label: string; items: Conversation[] }[] {
